/**
 * API Route: Hold items in cart
 * POST /api/viator/bookings/hold
 * 
...
export async function POST(request: NextRequest) {
  try {
    ...
    const holdRequest: CartBookingRequest = {
      items,
      bookerInfo,
      communication,
    }

    const result = await ViatorBookingService.holdCart(
      holdRequest,
      currency || 'USD',
      language || 'en-US'
    )
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to hold cart',
      },
      { status: 500 }
    )
  }
